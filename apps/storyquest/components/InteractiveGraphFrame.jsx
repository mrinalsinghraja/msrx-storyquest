...
/**
 * Scene colour, by state.
 *
 * The three states used to be drawn in two colours: `neutral` and `success`
 * were both `#00c4df`, so the single most important moment in a mission — the
 * two sides finally agreeing — changed a word in a pill and nothing else. A
 * learner scanning the apparatus had no way to see it.
 *
 * The controlled quantity now wears its world's colour at all times, and the
 * given quantity is inert slate — so the apparatus answers "which of these can
 * I move?" before it answers anything else. A first attempt made the controlled
 * side slate too until it balanced, which read well as a reward and destroyed
 * that first question: both blocks were the same grey and neither looked like
 * the learner's.
 *
 * Solving is signalled on the casing instead of by recolouring the scene, which
 * leaves the geometry untouched and is far more visible than a word changing
 * inside a pill.
 *
 * Failure is the one state that does repaint the scene, because a control
 * pushed somewhere the relationship cannot hold is worth interrupting for.
 *
 * Plain hex rather than the `oklch()` the stylesheet uses: these are handed to
 * SVG presentation attributes, which take a colour value and cannot resolve a
 * custom property.
 */
const WORLD_ACCENT = {
  physics: '#0f74c5',
...
const NEUTRAL_ACCENT = '#7b8799';
const FAILURE_ACCENT = '#cb4a2a';

/**
 * The fixed side of every relationship.
 *
 * Each scene draws two quantities: the one the learner controls, in the state
 * colour above, and the one the mission hands them. The given side used to be
 * brand purple in all 49 scenes, which made two saturated colours compete for
 * attention and told the learner nothing about which of them they could move.
 * Slate is deliberately inert — colour on this apparatus means "this is yours".
 */
const FIXED_SIDE = '#454e5e';

/**
 * The status pill — is this rig tuning, balanced, or out of range?
 *
 * This used to be a full title bar: a grey slab carrying the lab's name on the
 * left and the status on the right. Once the apparatus was mounted inside the
 * console casing, that slab named the machine directly below a rail that had
 * already named it, so every one of the 49 scenes wore two title bars. The name
 * belongs to the casing; only the state belongs to the screen.
 *
 * Kept inside the SVG rather than promoted to the console rail because it is
 * derived from `reading.balanced` — the same number the scene draws — and
 * splitting the two across a component boundary invites them to disagree.
 */
function LabStatus({ status, accent, glow }) {
  return (
    <g>
      <rect x="449" y="28" width="149" height="26" rx="13" fill={glow} stroke={accent} />
      <circle cx="468" cy="41" r="4" fill={accent} />
      <text x="533" y="45" fill="#1d1d1f" fontSize="11" fontWeight="700" textAnchor="middle" letterSpacing="0.6">{status}</text>
    </g>
  );
}
  ...
  return (
    <>
      ...
      <g transform={`rotate(${rotation} 320 218)`}>
        ...
        <rect x="168" y={210 - leftHeight} width="76" height={leftHeight} rx="10" fill="#454e5e" />
        <rect x="396" y={210 - rightHeight} width="76" height={rightHeight} rx="10" fill={accent} />
        <line x1="206" y1={192 - leftHeight} x2="206" y2="199" stroke="#454e5e" strokeWidth="4" markerEnd={`url(#${arrowId})`} />
        <line x1="434" y1={192 - rightHeight} x2="434" y2="199" stroke={accent} strokeWidth="4" markerEnd={`url(#${arrowId})`} />
      </g>
      <path d="M320 232L286 286H354Z" fill="#454e5e" />
...
function CircuitLab({ t, value, target, accent, arrowId, leftLabel, rightLabel, controlText, leftText }) {
  ...
  return (
    <>
      ...
      <path d="M252 104V248" fill="none" stroke="#454e5e" strokeWidth="5" strokeLinecap="round" />
...
function LensLab({ t, value, target, accent, arrowId, controlText, leftText }) {
  ...
  return (
    <>
      ...
      <line x1="190" y1="248" x2="190" y2="126" stroke="#454e5e" strokeWidth="6" markerEnd={`url(#${arrowId})`} />
      <path d="M320 96C282 137 282 211 320 252C358 211 358 137 320 96Z" fill="#cffafe" fillOpacity="0.55" stroke={accent} strokeWidth="3" />
      <line x1="190" y1="126" x2="320" y2="154" stroke="#454e5e" strokeWidth="3" />
      <line x1="320" y1="154" x2="540" y2={focusX > 320 ? 235 : 112} stroke={accent} strokeWidth="3" />
      <line x1="190" y1="126" x2="320" y2="126" stroke="#454e5e" strokeWidth="3" />
...
function WaveLab({ t, value, target, accent, controlText, leftText }) {
  ...
  return (
    <>
      ...
      <circle cx="320" cy={height(40)} r="7" fill="#454e5e" />
      <circle cx="74" cy={height(0)} r="11" fill="#454e5e" />
...
function ParticleLab({ t, value, accent, controlText, leftText }) {
  ...
  return (
    <>
      ...
      {dots.map((dot, index) => <circle key={index} cx={dot.x} cy={dot.y} r={dot.r} fill={index % 2 ? accent : '#454e5e'} opacity="0.9" />)}
...
function ReactionLab({ t, value, target, accent, arrowId, controlText, leftText }) {
  ...
  return (
    <>
      ...
      <line x1="320" y1="258" x2="320" y2={peak} stroke="#454e5e" strokeWidth="3" strokeDasharray="5 5" markerEnd={`url(#${arrowId})`} />
      <circle cx={walker.x} cy={walker.y} r="10" fill="#ffffff" stroke={accent} strokeWidth="3" />
      {[{ x: 144, y: 186, color: '#454e5e' }, { x: 184, y: 206, color: accent }, { x: 458, y: 186, color: accent }, { x: 498, y: 206, color: '#454e5e' }].map((molecule, index) => <g key={index}><circle cx={molecule.x} cy={molecule.y} r="18" fill={molecule.color} /><circle cx={molecule.x + 21} cy={molecule.y + 11} r="12" fill={molecule.color} opacity="0.75" /></g>)}
...
function AtomLab({ t, value, accent, controlText, leftText }) {
  ...
  return (
    <>
      ...
      <circle cx="320" cy="178" r="30" fill="#454e5e" opacity="0.92" />
...
function RatioLab({ t, value, target, accent, controlText, leftText }) {
  ...
  return (
    <>
      <path d="M102 250L202 120L302 250Z" fill="#ede9fe" stroke="#454e5e" strokeWidth="3" />
      <path d={`M338 250L${338 + targetSpan / 2} 120L${338 + targetSpan} 250Z`} fill="#e0f2fe" stroke={accent} strokeWidth="3" />
      <line x1={102 + sweep * 200} y1="120" x2={102 + sweep * 200} y2="250" stroke="#454e5e" strokeWidth="2" opacity="0.5" />
      <line x1={338 + sweep * targetSpan} y1="120" x2={338 + sweep * targetSpan} y2="250" stroke={accent} strokeWidth="2" opacity="0.5" />
      <line x1="102" y1="272" x2="302" y2="272" stroke="#454e5e" strokeWidth="3" />
...
function CoordinateLab({ t, value, accent, controlText, leftText }) {
  ...
  return (
    <>
      ...
      <circle cx={162 + p * 316} cy={240 + p * (endY - 240)} r="6" fill="#454e5e" opacity="0.85" />
      <circle cx="162" cy="240" r="8" fill="#454e5e" />
      <circle cx="478" cy={endY} r="8" fill={accent} />
      <path d={`M478 240V${endY}H162`} fill="none" stroke="#454e5e" strokeWidth="2" strokeDasharray="5 5" />
...
function GeometryLab({ t, value, accent, controlText, leftText }) {
  ...
  return (
    <>
      <path d="M320 240L176 240" stroke="#454e5e" strokeWidth="7" strokeLinecap="round" />
...
function DataLab({ t, value, target, accent, controlText, leftText }) {
  ...
  return (
    <>
      ...
      <circle cx={145 + head * 54} cy="284" r="4" fill="#454e5e" />
      <line x1="116" y1={meanY} x2="524" y2={meanY} stroke="#454e5e" strokeWidth="3" strokeDasharray="7 6" />
...
function CellLab({ t, value, accent, arrowId, controlText, leftText }) {
  ...
  return (
    <>
      ...
      />
      <circle cx="350" cy="185" r="36" fill="#454e5e" />
...
function EcosystemLab({ t, value, accent, arrowId, controlText, leftText }) {
  ...
  const nodes = [{ x: 160, y: 238, label: 'PLANTS', color: '#65a30d' }, { x: 320, y: 174, label: 'GRAZERS', color: '#454e5e' }, { x: 480, y: 118, label: 'PREDATORS', color: accent }];
...
function LungsLab({ t, value, accent, controlText, leftText }) {
  ...
  return (
    <>
      ...
      <path d={trace} fill="none" stroke="#454e5e" strokeWidth="4" />
...
function OsmosisLab({ t, value, accent, arrowId, controlText, leftText }) {
  ...
  return (
    <>
      ...
      />
      {dotGroup(outside, 128, '#454e5e', 0)}{dotGroup(inside, 380, accent, 7)}
...
function FrictionLab({ t, value, accent, arrowId, leftLabel, rightLabel, controlText }) {
  ...
  return (
    <>
      ...
      <rect x={238 + shudder} y="176" width="150" height="92" rx="10" fill="#454e5e" />
      <text x={313 + shudder} y="228" fill="#ffffff" fontSize="12" fontWeight="800" textAnchor="middle">LOAD</text>
      <line x1="313" y1="120" x2="313" y2="170" stroke="#454e5e" strokeWidth="5" markerEnd={`url(#${arrowId})`} />
...
function ForceLab({ t, value, accent, arrowId, leftLabel, rightLabel, controlText }) {
  ...
  return (
    <>
      ...
      <rect x="248" y="164" width="146" height="86" rx="12" fill="#ede9fe" stroke="#454e5e" strokeWidth="3" />
      <text x="321" y="214" fill="#454e5e" fontSize="13" fontWeight="800" textAnchor="middle">MASS</text>
...
function CollisionLab({ t, value, accent, arrowId, leftLabel, rightLabel, controlText }) {
  ...
  return (
    <>
      ...
      <rect x={approach + shiftA} y="196" width="96" height="60" rx="10" fill="#454e5e" />
      ...
      <text x="176" y="180" fill="#454e5e" fontSize="10" fontWeight="700">{leftLabel}</text>
...
function EnergyLab({ t, value, accent, leftLabel, rightLabel, controlText }) {
  ...
  return (
    <>
      ...
      <rect x={522 - (404 - kinetic)} y="204" width={404 - kinetic} height="46" rx="10" fill="#454e5e" />
      ...
      <text x="118" y="196" fill="#454e5e" fontSize="11" fontWeight="700">{rightLabel}</text>
...
function ProjectileLab({ t, value, accent, controlText, leftText }) {
  ...
  return (
    <>
      ...
      <path d="M84 262L104 240L124 262Z" fill="#454e5e" />
      <circle cx={shell.x} cy={shell.y} r="8" fill="#454e5e" />
      <circle cx={104 + range} cy="262" r="9" fill={accent} />
      <line x1={104 + range} y1="262" x2={104 + range} y2="290" stroke={accent} strokeWidth="2" />
      <line x1="104" y1="290" x2={104 + range} y2="290" stroke="#454e5e" strokeWidth="2" markerEnd="none" />
      <text x={104 + range / 2} y="310" fill="#454e5e" fontSize="11" fontWeight="700" textAnchor="middle">RANGE {leftText}</text>
...
function OrbitLab({ t, value, accent, controlText, leftText }) {
  ...
  return (
    <>
      <circle cx="320" cy="192" r="30" fill="#454e5e" />
...
function HydraulicLab({ t, value, accent, leftLabel, rightLabel, controlText }) {
  ...
  return (
    <>
      ...
      <rect x="124" y={inputTop} width="78" height="16" rx="4" fill="#454e5e" />
      <rect x="396" y={outputTop - 16} width="142" height="16" rx="4" fill={accent} />
      <line x1="163" y1={inputTop} x2="163" y2={inputTop - 42} stroke="#454e5e" strokeWidth="5" />
      <text x="163" y={inputTop - 52} fill="#454e5e" fontSize="10" fontWeight="700" textAnchor="middle">{rightLabel}</text>
...
function MagnetLab({ t, value, accent, arrowId, leftLabel, controlText }) {
  ...
  return (
    <>
      ...
      {[126, 242].map((y, index) => (
        <path key={y} d={`M150 ${y}Q320 ${index ? y + 62 : y - 62} 490 ${y}`} fill="none" stroke="#454e5e" strokeWidth="3" strokeDasharray="7 6" strokeDashoffset={march} markerEnd={`url(#${arrowId})`} />
      ))}
      <path d="M120 184H210" stroke="#454e5e" strokeWidth="4" markerEnd={`url(#${arrowId})`} />
      <text x="320" y="106" fill="#454e5e" fontSize="10" fontWeight="700" textAnchor="middle">{leftLabel}</text>
...
function MirrorLab({ t, value, accent, controlText }) {
  ...
  return (
    <>
      ...
      <line x1={320 - Math.sin(radians) * reach} y1={254 - Math.cos(radians) * reach} x2="320" y2="254" stroke="#454e5e" strokeWidth="4" />
      ...
      <text x={320 - Math.sin(radians) * reach - 6} y={248 - Math.cos(radians) * reach} fill="#454e5e" fontSize="10" fontWeight="700" textAnchor="end">INCIDENT</text>
...
function GasLab({ t, value, accent, arrowId, leftLabel, rightLabel, controlText, leftText }) {
  ...
  return (
    <>
      ...
      <rect x="196" y={286 - height} width="176" height="14" rx="4" fill="#454e5e" />
      <line x1="284" y1={280 - height} x2="284" y2="112" stroke="#454e5e" strokeWidth="6" />
      <line x1="284" y1="112" x2="284" y2="148" stroke="#454e5e" strokeWidth="6" markerEnd={`url(#${arrowId})`} />
...
function BondLab({ t, value, accent, controlText, leftText }) {
  ...
  return (
    <>
      <circle cx={212 - stretch} cy="188" r="52" fill="#454e5e" />
...
function SolutionLab({ t, value, accent, leftLabel, rightLabel, controlText, leftText }) {
  ...
  return (
    <>
      ...
      <line x1="446" y1={298 - level} x2="492" y2={298 - level} stroke="#454e5e" strokeWidth="3" strokeDasharray="6 5" />
      <text x="498" y={302 - level} fill="#454e5e" fontSize="10" fontWeight="700">{rightLabel}</text>
...
function RedoxLab({ t, value, accent, arrowId, leftLabel, rightLabel, controlText }) {
  ...
  return (
    <>
      <rect x="112" y="140" width="128" height="150" rx="10" fill="#faf5ff" stroke="#454e5e" strokeWidth="3" />
      <rect x="400" y="140" width="128" height="150" rx="10" fill="#ecfeff" stroke={accent} strokeWidth="3" />
      <rect x="168" y="112" width="16" height="120" rx="4" fill="#454e5e" />
      ...
      <text x="176" y="312" fill="#454e5e" fontSize="10" fontWeight="700" textAnchor="middle">{leftLabel}</text>
...
function ChromatographyLab({ t, value, accent, leftLabel, rightLabel, controlText, leftText }) {
  ...
  return (
    <>
      ...
      <line x1="252" y1="112" x2="388" y2="112" stroke="#454e5e" strokeWidth="3" />
      ...
      <text x="240" y="116" fill="#454e5e" fontSize="10" fontWeight="700" textAnchor="end">{rightLabel}</text>
...
function FractionLab({ t, value, accent, controlText, leftText }) {
  ...
  return (
    <>
      ...
      <line x1="320" y1="178" x2={320 + Math.cos(hand) * 88} y2={178 + Math.sin(hand) * 88} stroke="#454e5e" strokeWidth="2.5" opacity="0.75" />
      <circle cx="320" cy="178" r="5" fill="#454e5e" />
...
function TriangleLab({ t, value, accent, leftLabel, controlText, leftText }) {
  ...
  return (
    <>
      <path d={`M156 ${baseY}H436V${baseY - rise}Z`} fill="#ede9fe" stroke="#454e5e" strokeWidth="3" />
      ...
      <line x1="156" y1={baseY} x2="436" y2={baseY - rise} stroke="#454e5e" strokeWidth="5" />
      ...
      <text x="270" y={baseY - rise / 2 - 12} fill="#454e5e" fontSize="11" fontWeight="700" textAnchor="middle">{leftText}</text>
      <text x="320" y="112" fill={accent} fontSize="15" fontWeight="700" textAnchor="middle">{controlText}</text>
    </>
  );
}
...
function CircleLab({ t, value, accent, controlText, leftText }) {
  ...
  return (
    <>
      ...
      <line x1="320" y1="188" x2={320 + Math.cos(arm) * radius} y2={188 + Math.sin(arm) * radius} stroke="#454e5e" strokeWidth="4" />
      <circle cx={320 + Math.cos(arm) * radius} cy={188 + Math.sin(arm) * radius} r="7" fill="#454e5e" />
      <circle cx="320" cy="188" r="6" fill="#1d1d1f" />
      <text x={320 + Math.cos(arm) * radius / 2} y={182 + Math.sin(arm) * radius / 2} fill="#454e5e" fontSize="11" fontWeight="700" textAnchor="middle">r</text>
      <text x="320" y="316" fill={accent} fontSize="15" fontWeight="700" textAnchor="middle">{controlText}</text>
...
function AreaLab({ t, value, accent, leftLabel, controlText, leftText }) {
  ...
  return (
    <>
      ...
      <line x1="120" y1="292" x2={120 + width} y2="292" stroke="#454e5e" strokeWidth="3" />
      <line x1="104" y1="132" x2="104" y2="272" stroke="#a1a1a6" strokeWidth="3" />
      <text x={120 + width / 2} y="312" fill="#454e5e" fontSize="11" fontWeight="700" textAnchor="middle">{leftLabel}</text>
...
function VolumeLab({ t, value, accent, controlText, leftText }) {
  ...
  return (
    <>
      ...
      <line x1="182" y1="292" x2="182" y2={top} stroke="#454e5e" strokeWidth="3" />
      <text x="168" y={top + height / 2} fill="#454e5e" fontSize="11" fontWeight="700" textAnchor="end">h</text>
...
function NumberLineLab({ t, value, accent, controlText, leftText }) {
  ...
  return (
    <>
      ...
      <line x1="320" y1="172" x2="320" y2="220" stroke="#454e5e" strokeWidth="3" />
...
function BalanceLab({ t, value, target, accent, leftLabel, rightLabel, controlText }) {
  ...
  return (
    <>
      <path d="M320 246L282 296H358Z" fill="#454e5e" />
      ...
      <g transform={`rotate(${tilt} 320 236)`}>
        ...
        <line x1="164" y1="242" x2="164" y2="276" stroke="#454e5e" strokeWidth="3" />
        <path d="M124 276h80l-13 24h-54Z" fill="#454e5e" opacity="0.85" />
      ...
      <text x="164" y="152" fill="#454e5e" fontSize="11" fontWeight="700" textAnchor="middle">{leftLabel}</text>
  ...
  return (
    <>
      ...
      {Array.from({ length: 6 }, (_, index) => {
        ...
        return (
          <g key={index}>
            <rect x={128 + index * 70} y={292 - height} width="54" height={height} rx="5" fill={index % 2 ? accent : '#454e5e'} opacity={index === active ? 1 : 0.6} />
...
function ProbabilityLab({ t, value, accent, controlText, leftText }) {
  ...
  return (
    <>
      ...
      <path d="M320 62l14 26h-28Z" fill="#454e5e" />
...
function MitosisLab({ t, value, accent, controlText, leftText }) {
  ...
  return (
    <>
      ...
      <ellipse cx={320 + split} cy="180" rx={62 - split * 0.35} ry="62" fill="#d1fae5" fillOpacity="0.6" stroke="#10b981" strokeWidth="4" />
      <circle cx={320 - split + nudge} cy={180 - nudge} r="20" fill="#454e5e" />
...
function DnaLab({ t, value, accent, controlText, leftText }) {
  ...
  return (
    <>
      <path d={strand(0)} fill="none" stroke="#454e5e" strokeWidth="5" />
...
function WaterCycleLab({ t, value, accent, arrowId, leftLabel, rightLabel, controlText }) {
  ...
  return (
    <>
      ...
      <path d="M456 236V160" stroke="#454e5e" strokeWidth="4" strokeDasharray="7 6" strokeDashoffset={cycle(t, 2.2) * 13} markerEnd={`url(#${arrowId})`} />
      <text x="256" y="252" fill={accent} fontSize="10" fontWeight="700" textAnchor="middle">{leftLabel}</text>
      <text x="500" y="152" fill="#454e5e" fontSize="10" fontWeight="700" textAnchor="middle">{rightLabel}</text>
      <text x="320" y="326" fill={accent} fontSize="15" fontWeight="700" textAnchor="middle">{controlText}</text>
    </>
  );
}
...
function NeuronLab({ t, value, accent, arrowId, controlText, leftText }) {
  ...
  return (
    <>
      <circle cx="160" cy="188" r="46" fill="#ede9fe" stroke="#454e5e" strokeWidth="4" />
      <circle cx="160" cy="188" r="16" fill="#454e5e" />
      {[-1, 0, 1].map((offset) => <path key={offset} d={`M124 ${188 + offset * 30}l-42 ${offset * 26 - 4}`} stroke="#454e5e" strokeWidth="4" strokeLinecap="round" />)}
...
function DigestionLab({ t, value, accent, arrowId, leftLabel, rightLabel, controlText }) {
  ...
  return (
    <>
      ...
      <path d="M354 196h50" stroke="#454e5e" strokeWidth="4" markerEnd={`url(#${arrowId})`} />
      <text x="300" y="94" fill="#6e6e73" fontSize="10" fontWeight="700" textAnchor="middle">{leftLabel}</text>
      <text x="414" y="190" fill="#454e5e" fontSize="10" fontWeight="700">{rightLabel}</text>
...
export default function InteractiveGraphFrame({
  ...
  onChange,
  labKind,
  /** Which of the four worlds this mission belongs to; picks the solved colour. */
  world,
  disabled = false,
}) {
  ...
  const solvedAccent = WORLD_ACCENT[world] ?? WORLD_ACCENT.physics;
  const palette = {
    neutral: { stroke: solvedAccent, glow: 'rgba(123, 135, 153, 0.12)', label: 'TUNING' },
    success: { stroke: solvedAccent, glow: `${solvedAccent}26`, label: 'BALANCED' },
    failure: { stroke: FAILURE_ACCENT, glow: 'rgba(203, 74, 42, 0.14)', label: 'OUT OF RANGE' },
  }[effectiveStatus];
    ...
    <section className="relative w-full overflow-hidden rounded-2xl border border-[var(--border-strong)] bg-white" aria-label={lab.title}>
      <div className="aspect-video w-full">
        <svg viewBox="0 0 640 360" preserveAspectRatio="xMidYMid meet" className="h-full w-full" role="img" aria-labelledby={titleId}>
          ...
          <LabStatus status={palette.label} accent={palette.stroke} glow={palette.glow} />
          <LabScene
            kind={kind}
            t={t}
      ...
      <div className="border-t border-[var(--border)] bg-[var(--surface)] px-4 py-3">
        ...
        <input
          ...
          /* The control wears the same colour as the quantity it moves. It was
           * brand cyan on all 49 rigs, so the one thing the learner can actually
           * touch was the only element not participating in the world's palette. */
          style={{ accentColor: palette.stroke }}
          className="h-2 w-full cursor-pointer appearance-none rounded-full bg-[#e5e5ea] disabled:cursor-not-allowed disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--pigment-ink)]"
        />
      </div>
